...
{
    //copy
    copyFile("test_to_submiss/test/solution_you/output" + to_string(i) + ".txt", "test_fail/solution_you.txt");
    copyFile("test_to_submiss/test/solution/output" + to_string(i) + ".txt", "test_fail/solution.txt");
    copyFile("test_to_submiss/test/input/input" + to_string(i) + ".txt", "test_fail/input_fail.txt");

	string file_solution_you = "test_to_submiss/test/solution_you/output" + to_string(i) + ".txt";
	string file_solution = "test_to_submiss/test/solution/output" + to_string(i) + ".txt";
	...
	while (read_solution_you >> s1 && read_solution >> s2)
	{
		if (s1 != s2)
		{
			cout << "\nfail test_to_submiss/test " << i << " " << "https://www.diffchecker.com/text-compare/"; // << " : line " << k <<" in ouput"  << endl;
			return;
		}
		k++;
	}
	...
	{
		cout << "fail test_to_submiss/test " << i << " line " << k << endl;
	}
}

void comparefile(int start, int end)
{
	string folder_solution_you = "test_to_submiss/test/solution_you/output";
	string folder_solution = "test_to_submiss/test/solution/output";
	vector<int> result;
	...
	if (result.size() == 0)
	{
		cout << "Success: test_to_submiss/test " << start << " to " << end << endl;
	}
	else
	{
		cout << "percent success : " << (1 - result.size() * 1.0 / (end - start + 1)) * 100 << "%" << endl;
		cout << "Fail : test_to_submiss/test [";
...
int main(int argc, char *argv[])
{
	...
	string folder_input = "test_to_submiss/test/input/input";
	string folder_solution_you = "test_to_submiss/test/solution_you/output";
	string folder_solution = "test_to_submiss/test/solution/output";
