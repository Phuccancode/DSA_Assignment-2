...
//^ chú ý: cứ node bên trái thì mã hóa là '0', node bên phải mã hóa là '1'. thật tuyệt gõ code mà để unikey 
void encodingHuffman_rec(vector<string>& encoding, Node* node, string s = "")
{
    //TODO 
    if(node==nullptr) return;
    encoding[node->c]=s;
    encodingHuffman_rec(encoding,node->left,encoding[node->c]+"0");
    encodingHuffman_rec(encoding,node->right,encoding[node->c]+"1");
}
